<?php
    ...
    //Eliminar Producto
    //* * *
    if (isset($_POST["lospollitosdicencred"])){

        $id = $_POST["lospollitosdicencred"];
        $usuario = $_POST["piopiopio"];

        include("conexion.php");
        //Delete de la tabla Producto
        $sql = "DELETE FROM comparador_producto_cred WHERE producto_cred_id = $id";

        //Delete de la tabla de Requisito
        $sql3 = "DELETE FROM comparador_requisito_cred WHERE producto_cred_id = $id";
        ...
        if ($stmt = mysqli_prepare($mysqli, $sql3)) {
            ...
            if ($stmt = mysqli_prepare($mysqli, $sql)) {
                ...
                header("Location: compty-admin-user_dashboard_cred.php?id=$idUser&password=$password&delete=666");
                exit;
            } else {
                //print_r(error_get_last());
                header("Location: compty-admin-user_dashboard_cred.php?id=$idUser&password=$password&delete=999");
                exit;
            }
        }
